...
class ViewController: UIViewController {
    ...
    private func initPlayer() {
        ...
        if playerTwo != nil && playerOne != nil {
             _playerOne.name = playerOne!
            _playerTwo.name = playerTwo!
            _game = Game(p1: Player(name: playerOne!), p2: Player(name: playerTwo!
            ), matchesCount: userDefaultsManager.integer(forKey: Player.MATCHESCOUNT_KEY))
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        case "display-settings-by-Game":
            let settingVC = segue.destination as! SettingsViewController
            settingVC.game = _game
             print("i m in segue")
        default:
            print("nothing to do with this segue")
        }
    }
}
